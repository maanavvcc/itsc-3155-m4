...
def task3():
    ...
    credits_needed = credits_to_target(float(GLOBALS["gpa"]), float(target_gpa), int(GLOBALS["credits"]))
    # TODO 8: Return the data to the template
    return render_template('task3.html', name=GLOBALS["name"], age = GLOBALS["age"], 
                           major=GLOBALS["major"], credits=GLOBALS["credits"], gpa=GLOBALS["gpa"], 
                           target_gpa=target_gpa, credits_needed=credits_needed)
